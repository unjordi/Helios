...
| 24 | **Un PR de solo-docs/memoria dispara el build completo de ~16 min** (visto con el PR #18, tras activar el check requerido). ⚠️ **Trampa:** NO se arregla con un `paths-ignore` a secas — si el check requerido no corre, nunca reporta y el PR queda bloqueado PARA SIEMPRE. La salida correcta es un job *no-op* con el **mismo nombre** para las rutas ignoradas, o vivir con los 16 min. | Baja | — |
