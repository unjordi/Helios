...
namespace config {
  ...
  struct audio_t {
    ...
    bool keep_default;  ///< Keep the current default audio device instead of switching to the virtual sink.
    bool auto_capture;  ///< Automatically pick the audio device to capture when none is configured.
  };
  ...
  struct input_t {
    ...
    bool enable_input_only_mode;  ///< Allow clients to connect for input forwarding only (no video/audio).
    bool forward_rumble;  ///< Forward gamepad rumble/haptics back to the client.
  };
  ...
  /**
   * @brief Named command exposed to clients through the Web UI / server command API.
   */
  struct server_cmd_t {
    /**
     * @brief Build a server command entry from parsed configuration data.
     *
     * @param cmd_name Display name of the command.
     * @param cmd_val Command line to execute.
     * @param elevated Whether the command should run with elevated privileges.
     */
    ...
    std::string cmd_name;  ///< Display name of the command.
    std::string cmd_val;  ///< Command line to execute.
    bool elevated;  ///< Whether the process should be launched elevated.
  };
   ...
   */
  struct sunshine_t {
    bool hide_tray_controls;  ///< Hide the extra controls in the system tray menu.
    bool enable_pairing;  ///< Allow new clients to pair with this host.
    bool enable_discovery;  ///< Advertise this host over mDNS/network discovery.
    bool envvar_compatibility_mode;  ///< Also export legacy-named environment variables to child processes.
    ...
    bool legacy_ordering;  ///< Preserve the legacy ordering of the application list.
    ...
    std::vector<prep_cmd_t> state_cmds;  ///< Commands executed on host state transitions (connect/disconnect).
    std::vector<server_cmd_t> server_cmds;  ///< Named commands exposed to clients through the server command API.
