...
#undef _CONVERT
  ...
  /**
   * @brief Get the local IP address used to reach the default gateway.
   *
   * @return Local IP address string, or an empty string when it cannot be resolved.
   */
  ...
  constexpr auto SERVICE_NAME = "Apollo";  ///< Product/service name of the host (kept as "Apollo" for compatibility with existing installs).
  ...
  /**
   * @brief Read the host clipboard contents (Apollo/Helios clipboard sync).
   * @return Clipboard text, or an empty string on failure.
   */
  std::string get_clipboard();

  /**
   * @brief Replace the host clipboard contents (Apollo/Helios clipboard sync).
   * @param content Text to place on the host clipboard.
