...
#undef _CONVERT_2_ARG_
  ...
  input_t input {
    ...
    false,  // key_rightalt_to_key_win (also force-reset in apply_config)
    true,  // mouse enabled
  ...
  /**
   * @brief Consume the JSON server-command list setting.
   *
   * @param vars Parsed configuration entries; consumed keys are erased.
   * @param name Configuration key to consume.
   * @param input Destination field updated when the setting exists and parses successfully.
  ...
  void log_config_settings(const std::unordered_map<std::string, std::string> &vars, bool save) {
    ...
    #ifdef _WIN32
      BOOST_LOG(info) << "config: '"sv << name << "' = "sv << (is_redacted ? "[redacted]"s : utf8ToAcp(val));
    #else
      BOOST_LOG(info) << "config: '"sv << name << "' = "sv << (is_redacted ? "[redacted]"s : val);
    #endif
  ...
  int parse(int argc, char *argv[]) {
    ...
    if (!config_loaded && !shortcut_launch) {
      BOOST_LOG(fatal) << "To relaunch Helios successfully, use the shortcut in the Start Menu. Do not run sunshine.exe manually."sv;
