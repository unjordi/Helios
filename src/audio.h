...
namespace audio {
  ...
  struct config_t {
    ...
    // Apollo/Helios: `input_only` placed directly after `flags` was being overwritten to true,
    // so keep this padding between them. Do not remove without re-testing input-only sessions.
    uint64_t __padding;  ///< Padding guarding `input_only` from stray writes past `flags`.

    bool input_only;  ///< Session streams input only (no audio/video capture from this host app).
  };
