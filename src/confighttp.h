...
namespace confighttp {
  ...
  /**
   * @brief Lifetime of a Web UI session cookie handed out by the login endpoint.
   */
  ...
  /**
   * @brief Authenticate a Web UI request.
   *
   * Accepts either the session cookie handed out by `/api/login` (the Helios login flow)
   * or HTTP Basic credentials (upstream Sunshine behaviour, kept for API clients).
   *
   * @param response HTTP response object to populate on failure.
   * @param request HTTP request data from the client.
   * @param needsRedirect When true, an unauthenticated *browser navigation* is redirected to
   *                      the login page instead of receiving a 401.
   * @return True when the request is authenticated.
   */
  bool authenticate(const resp_https_t &response, const req_https_t &request, bool needsRedirect = false);
