...
namespace video {
   ...
   */
  struct config_t {
    // Only append new fields at the END of this struct; several call sites build it
    // field-by-field and Apollo's extra fields are expected to stay last.
    ...
    // Apollo/Helios additions
    int encodingFramerate;  ///< Requested display framerate (millifps) used for encode pacing.
    bool input_only;  ///< Session streams input only; no video is captured or encoded.
  };
