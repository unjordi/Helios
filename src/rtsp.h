...
namespace rtsp_stream {
  ...
  struct launch_session_t {
    ...
    std::string device_name;  ///< Friendly name reported by the paired client.
    std::string unique_id;  ///< Moonlight client unique identifier for this launch request.
    crypto::PERM perm;  ///< Per-client permission mask granted to this launch session.

    bool input_only;  ///< Whether the client joined in input-only (no video) mode.
    ...
    bool virtual_display;  ///< Whether the client asked for a virtual display.
    uint32_t scale_factor;  ///< Client-requested display scale factor.
    ...
    std::list<crypto::command_entry_t> client_do_cmds;  ///< Per-client commands run on connect.
    std::list<crypto::command_entry_t> client_undo_cmds;  ///< Per-client commands run on disconnect.

  #ifdef _WIN32
    GUID display_guid {};  ///< Virtual display GUID owned by this session (Windows only).
  #endif
  };
